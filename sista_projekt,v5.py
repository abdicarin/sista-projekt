...
def kasta_tarning():
    ...
    if kast == 1:
        ...
        info_label.config(text=f"Du fick en 1! Turen går till Spelare {2 if current_player == 0 else 1}")
        byt_spelare()
    else:
...
def behall_poang():
    ...
    player_scores[current_player] += current_round_score
    current_round_score = 0
    uppdatera_poang()

    if player_scores[current_player] >= 50:
        info_label.config(text=f"Spelare {current_player + 1} vinner!")
...
def byt_spelare(): #funktion av abdikariim
    ...
    current_round_score = 0
    info_label=Label(windows,text=f"Spelare {current_player + 1}'s tur")

def uppdatera_poang():
    score_label.config(text=f"Spelare 1: {player_scores[0]}  Spelare 2: {player_scores[1]}")
